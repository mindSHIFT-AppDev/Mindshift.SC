...
namespace Mindshift.SC.Common.Models {
			...
			if (File.Exists(serverPath)) {
				using (var sr = new StreamReader(serverPath)) {
					...
					foreach (Match match in matches) {
						...
						ret.Add(new ResponsePlaceholder(contextItem, layoutDefinition, match.Groups[1].Value, parentUniqueId, true));
					}
	...
	public class ResponsePlaceholder {
		...
		public ResponsePlaceholder(Item contextItem, LayoutDefinition layoutDefinition, string name, string parentUniqueId, bool exists = true) {
			Name = name;
	...
	public class ResponseRendering {
		...
		public ResponseRendering(RenderingReference renderingReference, LayoutDefinition layoutDefinition, Item contextItem) {
			...
			if (!string.IsNullOrWhiteSpace(DataSourceId)) {
				...
				if (datasourceItem != null) {
					DataSourcePath = datasourceItem.Paths.FullPath; // TODO: verify thiss
			...
			// TODO: ignore other parts of the path? 
			if (!string.IsNullOrWhiteSpace(PlaceholderPath)) {
				var arrPlaceholderPathList = PlaceholderPath.Split("/".ToCharArray());
				var placeholder = arrPlaceholderPathList.Last(); // 
				if (placeholder.Contains("~")) { // if the last one has a GUID, ignore the whole path - it's irrelevant - in fact, this will "fix" it.
					// assign unique id to this
					var temp = placeholder.Split("~".ToCharArray());

					PlaceholderName = temp[0];

					string strGuid = temp[1];
					try {
					...
					PlaceholderName = placeholder; // TODO: what's the GUID here?
		...
		//	var responsePlaceholder = new ResponsePlaceholder(name, responseRendering.UniqueId, true);
		//	_Placeholders.Add(responsePlaceholder);
		//	return responsePlaceholder;
		//}
		//internal List<ResponsePlaceholder> AddRenderingRange(List<ResponsePlaceholder> responseRenderingList) {
		//	_Renderings.AddRange(responseRenderingList);
		//	return responseRenderingList;
		//}
	}
